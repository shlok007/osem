...
describe 'User' do
  describe 'Abilities' do
    ...
    shared_examples 'user with any role' do
      ...
      it{ should_not be_able_to(:toggle_organization_role, Role.find_by(name: 'organization_admin', resource: @other_organization)) }
      ...
      %w(organizer cfp  info_desk volunteers_coordinator).each do |role|
        it{ should_not be_able_to(:toggle_conference_roles, Role.find_by(name: role, resource: @other_conference)) }
    ...
    shared_examples 'user with non-organizer role' do |role_name|
      %w(organizer cfp  info_desk volunteers_coordinator).each do |role|
        if role == role_name
          it{ should be_able_to(:toggle_conference_roles, Role.find_by(name: role, resource: my_conference)) }
        else
          it{ should_not be_able_to(:toggle_conference_roles, Role.find_by(name: role, resource: my_conference)) }
        end
